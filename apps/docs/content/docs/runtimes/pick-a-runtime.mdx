...
graph TD
    ...
    B -->|Google ADK| L[Use Google ADK Runtime]
...
<Cards>
  ...
  <Card
    title="Google ADK"
    description="For Google's Agent Development Kit with streaming, tool calls, and multi-agent support"
...
- **Google ADK Runtime** → Built on `LocalRuntime` with ADK streaming adapter
...
- **[Google ADK Example](https://github.com/assistant-ui/assistant-ui/tree/main/examples/with-google-adk)** - Google ADK with Gemini agents
...
2. **Follow the specific guide**:
   ...
   - [Google ADK Integration](/docs/runtimes/google-adk)
