    ...
    const threadListAdapter: RemoteThreadListAdapter = {
      async list(params) {
        ...
        return {
          ...
          cursor: data.nextCursor, // enables load-more if present
        };
      },
...
<ParametersTable
  ...
  parameters={[
    {
      name: "list",
      type: "(params?: { cursor?: string }) => Promise<{ threads: RemoteThreadMetadata[]; cursor?: string }>",
      description:
        "Return threads, optionally paginated. Each thread must include status, remoteId, and any metadata you want to show immediately. Return a cursor to enable load-more functionality.",
      required: true,
    },
    {
      name: "initialize",
...
Custom Thread List supports cursor-based pagination for loading threads incrementally.

To enable pagination:

1. Return a `cursor` from your `list()` adapter when more threads are available
2. Accept the `cursor` parameter in `list()` to fetch the next page

The runtime exposes pagination state and actions:

```tsx
import { useAuiState, useAui } from "@assistant-ui/react";

// Check if more threads can be loaded
const canLoadMore = useAuiState((s) => s.threads.canLoadMore);

// Load the next page
const aui = useAui();
const handleLoadMore = () => aui.threads().loadMore();
```

When `canLoadMore` is `true`, calling `loadMore()` invokes your adapter's `list({ cursor })` method with the cursor from the previous response. The returned threads are appended to the existing list.
