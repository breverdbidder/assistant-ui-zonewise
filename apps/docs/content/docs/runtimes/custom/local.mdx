...
While LocalRuntime manages state in-memory by default, it offers multiple persistence options - use the `storage` option for browser localStorage persistence, Assistant Cloud for managed multi-thread support, or implement your own storage with `useRemoteThreadListRuntime`.
...
- **Thread persistence** - Via localStorage, Assistant Cloud, or custom thread list adapter
...
For client-side persistence without a backend, pass `localStorage` to the storage option:
...
- Multiple threads persisted in the browser
- Thread management (create, switch, rename, archive, delete)
- Automatic title generation

Customize the storage prefix and title generation:
...
const runtime = useLocalRuntime(MyModelAdapter, {
  storage: localStorage,
  storagePrefix: "my-app-", // Custom key prefix (default: "assistant-ui-")
  titleGenerator: createSimpleTitleAdapter(), // Or implement your own
});
```

The `storage` option accepts any `AsyncStorageLike` object—compatible with `localStorage`, `sessionStorage`, or async backends like React Native's AsyncStorage.

### 3. Custom Database with useRemoteThreadListRuntime
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      ...
      description:
        "Storage backend for thread persistence (e.g., localStorage, sessionStorage, or React Native AsyncStorage)",
    },
    {
      name: "storagePrefix",
      type: "string",
    ...
    },
    {
      ...
      description:
        "Custom adapter for generating thread titles. Use createSimpleTitleAdapter() for a basic implementation",
    },
    {
      name: "adapters",
