...
### ThreadTokenUsage Type

`useThreadTokenUsage` returns a `ThreadTokenUsage` object:
...
| `reasoningTokens` | `number?` | Reasoning tokens (e.g., OpenAI o1, Claude extended thinking) |
| `cachedInputTokens` | `number?` | Cached input tokens (prompt caching) |

### getThreadMessageTokenUsage

Extracts token usage outside of React components:

```tsx
import { getThreadMessageTokenUsage } from "@assistant-ui/react-ai-sdk";

const message = { role: "assistant", metadata: { /* ... */ } };
const usage = getThreadMessageTokenUsage(message);
// Returns: ThreadTokenUsage | undefined
```
