...
By default, tool call results must be manually sent back. For agentic flows where tools should execute and continue automatically, use `sendAutomaticallyWhen` with `lastAssistantMessageIsCompleteWithToolCalls`:
