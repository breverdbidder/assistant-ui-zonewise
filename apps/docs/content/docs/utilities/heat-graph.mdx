...
`@assistant-ui/heat-graph` provides headless, Radix-style primitives for building GitHub-style activity heatmap graphs.
...
<Tabs items={["shadcn", "npm", "pnpm", "yarn"]}>
  <Tab value="shadcn">
    ...
    ```
    This installs a pre-styled `HeatGraph` component to `components/assistant-ui/heat-graph.tsx` along with the `@assistant-ui/heat-graph` package.
  </Tab>
  <Tab value="npm">
    ```sh
  ...
  <Tab value="pnpm">
    ```sh
    pnpm add @assistant-ui/heat-graph
    ```
  </Tab>
  <Tab value="yarn">
    ```sh
    yarn add @assistant-ui/heat-graph
    ```
  </Tab>
</Tabs>
...
import * as HeatGraph from "@assistant-ui/heat-graph";
