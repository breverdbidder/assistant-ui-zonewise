...
The ActionBar primitive provides message actions: copy, reload, edit, feedback, speech, export, and share. It handles intelligent visibility with auto-hide on hover, automatic disabling based on message state, and floating behavior. You compose the buttons; the primitive handles action state and availability.
