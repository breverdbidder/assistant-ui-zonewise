...
Renders all queue items in the composer. Queue items are messages submitted while the assistant is already running. Only renders when the runtime supports queuing (`capabilities.queue`).
...
The render function receives `{ queueItem }` with the queue item's `id` and `prompt`. Use `QueueItemPrimitive` parts inside to render the item's text and action buttons.

<Callout type="info">
See the [QueueItem primitive](/docs/primitives/queue-item) for full documentation on queue item parts and patterns.
</Callout>
...
When the runtime supports queuing, users can submit messages while the assistant is still running. Show queued messages and provide controls to steer or remove them:
...
When queue support is enabled, `Send` stays enabled during runs. Users can press <Kbd>Cmd/Ctrl + Shift + Enter</Kbd> to send with steer, which interrupts the current run and processes the new message immediately.

<Callout type="info">
Queue support requires a runtime with `capabilities.queue` enabled. See the [QueueItem primitive](/docs/primitives/queue-item) for setup and full documentation.
</Callout>
...
- [QueueItemPrimitive](/docs/primitives/queue-item)
