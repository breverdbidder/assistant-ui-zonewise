...
Register static system instructions in the model context.
...
Register dynamic context that is evaluated lazily at send-time. Use this for frequently-changing application state.

```tsx
import { useAssistantContext } from "@assistant-ui/react-native";

useAssistantContext({
  getContext: () => `Current screen: ${getCurrentScreen()}`,
});
```
