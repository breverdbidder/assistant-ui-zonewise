...
The example includes a bundled API endpoint for local development.

```sh
npx expo start
```

For production, create a `.env` file pointing to your own backend:

```
EXPO_PUBLIC_CHAT_ENDPOINT_URL="https://your-api.example.com/api/chat"
```

See [Manual Setup](#manual-setup) for creating your own backend endpoint.
