...
// send with steer (interrupt current run to process immediately)
...
// access queue items (when runtime supports queuing)
const queueItem = aui.composer().queueItem({ index: 0 });
queueItem.steer(); // Process this item immediately
...
type ComposerSendOptions = {
  steer?: boolean; // Interrupt current run to process this immediately (requires runtime queue support)
}
```

#### `QueueItemState`

State of a queued message waiting to be processed.
