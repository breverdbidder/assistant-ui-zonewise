...
| <Kbd>Cmd/Ctrl + Shift + Enter</Kbd> | Sends with steer (interrupts current run). Requires `capabilities.queue`. |
  ...
  parameters={[
    {
      ...
      required: true,
      description: "Render function called for each queue item. Receives the queue item state with `id` and `prompt`.",
    },
  ]}
/>

<Callout type="info">
See the [QueueItemPrimitive reference](/docs/api-reference/primitives/queue-item) for queue item parts and the [QueueItem guide](/docs/primitives/queue-item) for patterns.
</Callout>
