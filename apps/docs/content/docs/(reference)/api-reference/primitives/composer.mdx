...
| <Kbd>Ctrl/Cmd + Shift + Enter</Kbd> | Sends the message with steer (interrupts the current run to process this message immediately, when queue is supported). |
...
When the runtime supports message queuing, the Send button remains enabled during runs. Clicking it will enqueue the message. Without queue support, the button is disabled during runs.

To programmatically send with steer (interrupt the current run), use `composer.send({ steer: true })`.
...
Renders all queued messages in the composer. Only visible when the runtime supports message queuing.

Accepts a render function that is called for each queue item with `{ queueItem: QueueItemState }`.
...
<ParametersTable
  type="ComposerPrimitiveQueueProps"
  parameters={[
    {
      name: "children",
      type: "(value: { queueItem: QueueItemState }) => ReactNode",
      required: true,
      description: "A render function called for each queue item.",
    },
  ]}
/>

<Callout type="info">
See the QueueItemPrimitive documentation for components to render individual queue items.
</Callout>
