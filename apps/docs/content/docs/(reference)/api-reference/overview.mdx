...
- State: `useAuiState((s) => s.thread)`
  - `thread.capabilities.queue: boolean` - Indicates if the runtime supports message queuing and steering
...
- State: `useAuiState((s) => s.composer)`
  - `composer.queue: readonly QueueItemState[]` - Array of queued messages waiting to be processed
- Actions: `useAui().composer()`
  - `composer().send(options?: ComposerSendOptions)` - Send message, optionally with steer flag to interrupt current run
  - `composer().queueItem({ index: number })` - Get queue item runtime by index
...
>
  ...
  />
  <Component
    name="ComposerPrimitive.Queue"
    isContextProvider={true}
    providedContexts={[
      {
        name: "QueueItem Context",
        color: contextColors["QueueItem Context"],
      },
    ]}
    docsLink="#composer-primitive-queue"
    tooltip="Renders queued messages with a render function pattern"
  />
  <Component
...
<ContextLevel color={contextColors["QueueItem Context"]}>

## QueueItem Context

Provides access to individual queue items within the composer. Available when iterating through ComposerPrimitive.Queue.

### QueueItemRuntime

- State: `useAuiState((s) => s.queueItem)`
  - `queueItem.id: string` - Unique identifier for the queue item
  - `queueItem.prompt: string` - The message text queued for processing
- Methods: `useAui().queueItem()`
  - `queueItem().getState()` - Get the current queue item state
  - `queueItem().steer()` - Process this item immediately (interrupt current run)
  - `queueItem().remove()` - Remove this item from the queue

Scope: Available within ComposerPrimitive.Queue render function.

### QueueItemPrimitive

<Component
  name="QueueItemPrimitive.Text"
  docsLink="#queue-item-primitive-text"
  tooltip="Renders the queue item's prompt text"
/>

<Component
  name="QueueItemPrimitive.Steer"
  docsLink="#queue-item-primitive-steer"
  tooltip="Button to process this item immediately"
/>

<Component
  name="QueueItemPrimitive.Remove"
  docsLink="#queue-item-primitive-remove"
  tooltip="Button to remove from queue"
/>
