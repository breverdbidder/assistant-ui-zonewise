...
Use the `beforeReport` hook to enrich or filter telemetry. This requires the `assistant-cloud` package:
...
The example above uses anonymous mode (browser session-based user ID) via the env var. For production apps with user accounts, install the `assistant-cloud` package and pass an explicit cloud instance:

<InstallCommand npm={["assistant-cloud"]} />
