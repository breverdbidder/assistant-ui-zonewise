...
Use the `CreateAttachment` type to add attachments from external sources:
...
Pass this to `aui.composer().addAttachment()` to add attachments that bypass the `AttachmentAdapter` entirely.

See the [Attachments Guide](/docs/guides/attachments#external-source-attachments) for usage examples.
