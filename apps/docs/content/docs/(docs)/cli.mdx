...
Options:
  -p, --preset <name-or-url> preset name or URL (forwarded to 'assistant-ui create')
...
# Create with a preset (short name)
...
Presets apply pre-built configurations from the [assistant-ui playground](https://www.assistant-ui.com/playground). You can use short names like `chatgpt` instead of full URLs—the CLI expands them automatically:

```bash
# These are equivalent
npx assistant-ui create my-app --preset chatgpt
npx assistant-ui create my-app --preset "https://www.assistant-ui.com/playground/init?preset=chatgpt"
```

<Callout>
You cannot use `--preset` together with `--example`.
</Callout>
