...
Use the `agent` command to launch Claude Code with assistant-ui skills.

```bash
npx assistant-ui@latest agent <prompt>
```

This command:
- Launches Claude Code with assistant-ui knowledge bundled
- Requires Claude Code to be installed first

**Prerequisites**

Install Claude Code before using this command:
...
```bash
# Add a chat thread to your project
npx assistant-ui agent "add a chat thread to my project"

# Set up AI SDK runtime
npx assistant-ui agent "configure the AI SDK runtime with streaming"

# Add tool calling support
npx assistant-ui agent "add a weather tool with frontend UI"

# Preview what command would be run
npx assistant-ui agent "add a chat thread" --dry
```

**Bundled Skills**

The assistant-ui skill teaches Claude Code how to:
- Detect existing assistant-ui setup (`components.json` and `@assistant-ui/react`)
- Initialize new projects with `npx assistant-ui init --yes`
- Add components via the shadcn registry
- Set up AI SDK runtime with streaming
- Configure tool calling with backend and frontend components
