...
### Listening for Attachment Errors

Use the `composer.attachmentAddError` event to respond when an attachment fails to add:

```tsx
import { useAuiEvent } from "@assistant-ui/react";

function AttachmentErrorHandler() {
  useAuiEvent("composer.attachmentAddError", (payload) => {
    // payload includes threadId, and optionally messageId and attachmentId
    console.error("Attachment failed:", payload.attachmentId);
...
This event fires when an exception is thrown during the attachment add process, or when the adapter returns an attachment with an error status.
