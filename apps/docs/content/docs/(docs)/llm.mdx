...
Use the `Accept: text/markdown` header to request markdown content:

- `/` with `Accept: text/markdown` → redirects to `/llms.txt`
- `/docs/*` pages with `Accept: text/markdown` → redirects to the `.mdx` version

```bash
# Get the docs index
curl -H "Accept: text/markdown" https://www.assistant-ui.com/

# Get a specific page in markdown
curl -H "Accept: text/markdown" https://www.assistant-ui.com/docs/installation
```
