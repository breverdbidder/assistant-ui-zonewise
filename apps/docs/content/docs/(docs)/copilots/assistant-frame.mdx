...
Tool parameters accept any [StandardSchemaV1](https://github.com/standard-schema/standard-schema)-compatible schema library (like Zod v4+, Valibot, or ArkType) or plain JSON Schema.

```tsx
const handle = registry.addTool({
  toolName: string,
  description?: string,
  parameters: StandardSchemaV1 | JSONSchema,
