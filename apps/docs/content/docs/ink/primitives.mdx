...
When `status` is `requires-action` and both `addResult` and `resume` props are provided, ToolFallback automatically renders a [ToolApproval](/docs/ink/tool-approval) UI for human-in-the-loop workflows.
...
| `approvalProps` | `Partial<ToolApprovalProps>` | Props forwarded to ToolApproval |
