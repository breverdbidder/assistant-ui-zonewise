...
```tsx
import { DiffPrimitive, parsePatch } from "@assistant-ui/react-ink";

const files = parsePatch(patchString);

<DiffPrimitive.Root files={files}>
...
For custom layouts, parse the patch first with `parsePatch` then pass the result to `DiffPrimitive.Root`:
