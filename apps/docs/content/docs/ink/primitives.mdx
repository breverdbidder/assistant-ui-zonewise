...
Conditional rendering based on whether the message has an error status. Renders children only when the message has an error.
...
Container `Box` for error content. Only renders when the message has an error status.
...
`Text` component displaying the error message. Falls back to the error as a string if no children are provided. Only renders when the message has an error status.

```tsx
<ErrorPrimitive.Message />

{/* Or with custom content */}
<ErrorPrimitive.Message color="red">
...
| Prop | Type | Description |
|------|------|-------------|
| `children` | `ReactNode` | Optional custom error content |
| `...rest` | `TextProps` | Standard Ink Text props (e.g., `color`) |
