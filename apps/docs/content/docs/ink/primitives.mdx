...
Renders children only when the current message has an error.

```tsx
<MessagePrimitive.Error>
  <Text color="red">This message failed</Text>
</MessagePrimitive.Error>
```
...
Primitives for displaying error states when a message fails.

### Root

Container `Box` that renders only when the current message has an error.
...
`Text` displaying the error message. Only renders when an error exists. Without children, displays the error string directly.
...
| Prop | Type | Description |
|------|------|-------------|
| `children` | `ReactNode` | Optional custom content; defaults to the error message |
