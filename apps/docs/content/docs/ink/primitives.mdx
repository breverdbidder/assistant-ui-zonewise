...
**Keyboard shortcuts:**
- `Cmd/Ctrl+Shift+Enter` — Send with steer to interrupt the current run (when queue support is enabled)

### Send

`Box` that triggers sending the current message. Typically used with a button-like UI.

When the runtime supports message queuing, the Send button stays enabled during runs — pressing it adds the message to the queue. Without queue support, the button is disabled while running (existing behavior).
...
Renders all queued messages in the composer. Uses a render function pattern.
...
| Prop | Type | Description |
|------|------|-------------|
| `children` | `(value: { queueItem: QueueItemState }) => ReactNode` | Render function for each queue item |
...
Primitives for rendering individual queue items. Queue items are messages submitted while the agent is running. These components are used inside `ComposerPrimitive.Queue`.

### Text

`Text` component displaying the prompt text of a queue item.
...
Pressable that steers the current run to process this queue item immediately.

```tsx
<QueueItemPrimitive.Steer>
  <Text color="yellow">[Run Now]</Text>
</QueueItemPrimitive.Steer>
```

### Remove

Pressable that removes this item from the queue.
