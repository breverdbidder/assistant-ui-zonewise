...
Primitives support two import patterns.

### Namespace Imports (Default)
...
Use direct named imports with bundlers that perform static tree-shaking (e.g., `bun build --compile`). These bundlers may drop namespace re-exports like `MessagePrimitive.Root` but preserve direct imports like `MessageRoot`.
