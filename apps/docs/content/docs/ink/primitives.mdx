...
Renders the message list with automatic runtime integration. Each message is wrapped in a scoped context so that `useAuiState((s) => s.message)` works inside message components.
...
| Prop | Type | Description |
|------|------|-------------|
| `components` | `{ Message?, UserMessage?, AssistantMessage?, SystemMessage?, EditComposer?, ... }` | Component renderers by message role |

The `components` prop accepts either a single `Message` component for all roles, or separate `UserMessage`, `AssistantMessage`, and optionally `SystemMessage` components.
