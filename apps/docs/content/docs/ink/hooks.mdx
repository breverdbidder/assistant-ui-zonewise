...
Create an `AssistantRuntime` with a `ChatModelAdapter`. Supports [Assistant Cloud](/docs/cloud) integration for thread persistence.
...
const runtime = useLocalRuntime(chatModel, {
  initialMessages: [],
  cloud: assistantCloud, // optional - for cloud persistence
});
```
